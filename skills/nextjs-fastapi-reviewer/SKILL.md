...
2. Read the nextjs-fastapi-conventions skill. It defines the
...
Check against the architecture principles in the
nextjs-fastapi-conventions skill:
...
- Verify code follows all quality rules from the
  nextjs-fastapi-conventions skill (Python and TypeScript
  sections).
...
Apply all code quality rules from the nextjs-fastapi-conventions
skill: Python (backend) and TypeScript (frontend) sections.

### 6. Run the linters

Run lint checks using the commands from the
nextjs-fastapi-conventions skill. Confirm no issues are reported
for modified files.
